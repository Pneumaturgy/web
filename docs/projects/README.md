# Projects

Studio work and personal projects together, ordered by last commit, with
the projects that are still only design documents after them. Repository
figures were read from GitHub on 2 September 2026.

Only four of these were touched in the last week of August; two have not
moved since June. "Last commit" below means exactly that — none of it
implies all of them are live.

## With a repository

| Project | Repository | Last commit | Visibility |
| --- | --- | --- | --- |
| [Quire](quire.md) | [Ghigog/Quire](https://github.com/Ghigog/Quire) | 31 Aug 2026 | public |
| [By Jove](by-jove.md) | [Ghigog/by_jove_godot](https://github.com/Ghigog/by_jove_godot) | 31 Aug 2026 | private |
| [Vapor Music](vapor-music.md) | [Ghigog/vapor-music](https://github.com/Ghigog/vapor-music) | 31 Aug 2026 | public |
| [Seven Sages](seven-sages.md) | [Ghigog/Shipwright](https://github.com/Ghigog/Shipwright/tree/mod/seven-sages) | 13 Aug 2026 | public (fork) |
| [Lytra](lytra.md) | [Ghigog/lytra](https://github.com/Ghigog/lytra) | 25 Jul 2026 | public |
| [Mollusk](mollusk.md) | [Ghigog/mollusk](https://github.com/Ghigog/mollusk) | 16 Jul 2026 | private |
| [Navi](navi.md) | [Ghigog/navi-agent](https://github.com/Ghigog/navi-agent) | 7 Jul 2026 | private |
| [Orison](orison.md) | [Ghigog/orison](https://github.com/Ghigog/orison) | 17 Jun 2026 | private |
| [Pneuma Knights](pneuma-knights.md) | [Pneumaturgy/PorNada](https://github.com/Pneumaturgy/PorNada) | 1 Jun 2026 | private |
| [Theory Forge](theory-forge.md) | [Ghigog/lol-theory](https://github.com/Ghigog/lol-theory) | 1 Jun 2026 | public |
| [Arcade Tycoon](arcade-tycoon.md) — scaffold only | [Pneumaturgy/ArcadeTycoon](https://github.com/Pneumaturgy/ArcadeTycoon) | 30 Nov 2024 | private |
| [Trinks](trinks.md) — GMTK 2024 | [Pneumaturgy/GMTKjam](https://github.com/Pneumaturgy/GMTKjam) | 20 Aug 2024 | private |
| [Cerdo](cerdo.md) | [Ghigog/Cerdo](https://github.com/Ghigog/Cerdo) | 7 Jul 2024 | public |

Studio projects are Pneuma Knights, Arcade Tycoon, Trinks and Cerdo. The
rest are personal.

## Design docs, no repository yet

| Project | Status | Created |
| --- | --- | --- |
| [Geomancy](geomancy.md) | In development | 29 April 2024 |
| [Shryn](shryn.md) | In development | 3 June 2024 |
| [SciWars: Apocalypse](sciwars-apocalypse.md) | In development | 15 July 2024 |
| [PneuMath](pneumath.md) | Shipped in a ten-hour jam, repository not identified | 13 July 2024 |
| [LifeTracker](lifetracker.md) | Sketch | 20 August 2024 |
| [Ratchet & Clank Cleaner](ratchet-and-clank-cleaner.md) | Sketch (empty) | 20 August 2024 |

### Geomancy sub-pages

- [Original Concept](geomancy-original-concept.md)
- [Classes](geomancy-classes.md)

## Stories

All three are proposals for [Pneuma Knights](pneuma-knights.md). Por Nada
gives the game's repository its name.

| Story | Created |
| --- | --- |
| [Por Nada](por-nada.md) | 22 July 2024 |
| [Nico's Story Proposal](nicos-story-proposal.md) | 22 July 2024 |
| [Rapha's Story](raphas-story.md) | 23 July 2024 |

## One line of work, three bodies

[Navi](navi.md), [Mollusk](mollusk.md) and [Orison](orison.md) are the same
idea in different shapes: a local-model companion as a desktop fairy, a
living-room appliance, and a game master. They share the two-model local
pattern, and Navi's emotion system was reused in Orison.

## Documentation

In the [handbook](../handbook/): [project set
up](../handbook/project-set-up.md) and the [Godot mobile UI
template](../handbook/godot-mobile-ui-template.md).
